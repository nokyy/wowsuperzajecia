...
import { Actions, TodoStoreActions } from "../actions/todos_actions";
...
export const todosStoreReducer: Reducer<TodosList, Actions> = (state: TodosList = todosListInitialState, actions: Actions) => {
    switch(actions.type){
        ...
        case TodoStoreActions.SET_TODO_DONE:
            ...
            if(todoToChange){      
                ...
                return {
                    ...state,
                    list: [
                        ...state.list.filter(el=>el.id !== actions.payload.id), todoToChange
                    ]
                };
            }
            return state;
